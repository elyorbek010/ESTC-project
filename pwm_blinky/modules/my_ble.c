...
#define DEVICE_NAME "Elyorbek"                  /**< Name of device. Will be included in the advertising data. */
...
static ble_uuid_t m_adv_uuids[] = /**< Universally unique service identifiers. */
    {
        ...
        // Add ESTC service UUID to the table
...
APP_TIMER_DEF(char_2_notification_timer);
void char_2_notification_timeout_handler(void *p_context);

uint32_t rgb_ble;
ble_gatts_value_t ble_rgb_value;
...
{
    app_timer_create(&char_2_notification_timer, APP_TIMER_MODE_REPEATED, char_2_notification_timeout_handler);

    app_timer_start(char_2_notification_timer, APP_TIMER_TICKS(1000), NULL);
}
...
static void ble_evt_handler(ble_evt_t const *p_ble_evt, void *p_context)
{
    ...
    case BLE_GATTS_EVT_WRITE:
        ...
        if (p_ble_evt->evt.gatts_evt.params.write.handle == m_estc_service.char_1_handle.value_handle)
        {
            sd_ble_gatts_value_get(m_estc_service.connection_handle, m_estc_service.char_1_handle.value_handle, &ble_rgb_value);
            set_rgb((uint8_t)(rgb_ble >> 0), (uint8_t)(rgb_ble >> 8), (uint8_t)(rgb_ble >> 16));
        }
        break;
...
void char_2_notification_timeout_handler(void *p_context)
{
    estc_notify_char(&m_estc_service);
}
...
void ble_rgb_update()
{
    ...
    rgb_ble = (blue << 16) + (green << 8) + (red << 0);

    sd_ble_gatts_value_set(m_estc_service.connection_handle, m_estc_service.char_2_handle.value_handle, &ble_rgb_value);
};
